...
Dr = ENC(1:30:end,2) * MM_PER_PULSE; %Change here not to read all (1:10:end,1)
Dl = ENC(1:30:end,1) * MM_PER_PULSE;
N = max(size(Dr));
...
for kk=2:N
    ...
    % To do
    % Save values to see when the compensation has effect
...
figure; 
    ...
    hold on;
        for kk = 1:1:N % to plot every 10:th write 1:10:N
